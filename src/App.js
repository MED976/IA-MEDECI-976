...
function freshnessInfo(dateStr) {
  ...
  if (mins < 60) return { label: 'Fresh',      color: '#1A8917', bg: '#E8F5E9', bar: 60  };
  if (mins < 90) return { label: 'Still warm', color: '#5F9EA0', bg: '#E8F4F4', bar: 35  };
  return              { label: 'Fading',      color: '#ADADAD', bg: '#F5F5F5', bar: 15  };
}
...
const BUSINESS_TYPES = [
  { id: 'bakery',     icon: '🥖', label: 'Bakery',     gradient: 'linear-gradient(135deg,#FF8C42,#FF5000)' },
  { id: 'pizzeria',   icon: '🍕', label: 'Pizzeria',   gradient: 'linear-gradient(135deg,#FF6B6B,#C0392B)' },
  { id: 'pastry',     icon: '🥐', label: 'Pastry',     gradient: 'linear-gradient(135deg,#F8BBD0,#E91E63)' },
  { id: 'restaurant', icon: '🍽️', label: 'Restaurant', gradient: 'linear-gradient(135deg,#81C784,#2E7D32)' },
  { id: 'cafe',       icon: '☕', label: 'Café',        gradient: 'linear-gradient(135deg,#BCAAA4,#4E342E)' },
  { id: 'other',      icon: '🏪', label: 'Other',      gradient: 'linear-gradient(135deg,#B0BEC5,#546E7A)' },
];
...
export default function App() {
  ...
  const [venueScreen, setVenueScreen]     = useState('dashboard'); // dashboard | register
  ...
  const enriched = venues
    .map(v => ({
      ...
      bt: BUSINESS_TYPES.find(b => b.id === v.type),
    }))
  ...
  return (
    <div style={{ fontFamily: "'Inter', system-ui, sans-serif", maxWidth: 480, margin: '0 auto', minHeight: '100vh', background: C.bg, display: 'flex', flexDirection: 'column' }}>
        ...
        {(tab === 'explore' || tab === 'favorites') && (
          <>
            ...
            <div style={{ background: C.black, padding: '52px 20px 20px' }}>
              ...
              {tab === 'explore' && (
                <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: 12, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 15 }}>Bakeries, pizzerias, cafés…</span>
                </div>
              )}
            </div>
            ...
            <div style={{ padding: '0 16px' }}>
              {loading && (
                <div style={{ textAlign: 'center', padding: '60px 0', color: C.muted, fontSize: 14 }}>Loading fresh items…</div>
              )}
              {locError && (
                <div style={{ background: '#FDECEA', borderRadius: 14, padding: 16, marginBottom: 12 }}>
                  ...
                  <button onClick={() => { setLocError(''); getLocation(); }} style={btnStyle(C.primary)}>Try again</button>
                </div>
              )}
              {!loading && enriched.length === 0 && (
                <div style={{ textAlign: 'center', padding: '60px 20px' }}>
                  ...
                  <div style={{ fontSize: 20, fontWeight: 800, marginBottom: 8 }}>
                    {tab === 'favorites' ? 'No favorites yet' : 'Nothing fresh nearby'}
                  </div>
                  <div style={{ fontSize: 14, color: C.muted, lineHeight: 1.6 }}>
                    {tab === 'favorites' ? 'Save spots by tapping the ❤️ on any card.' : 'Items appear up to 2h after being announced.'}
                  </div>
                </div>
              )}
              ...
              {enriched.map(v => {
                ...
                const f = freshnessInfo(topItem.at);
                const q = QUANTITIES.find(q => q.id === topItem.quantity);
                return (
                  <div key={v.id} style={{ background: C.card, borderRadius: 20, marginBottom: 16, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
                    ...
                    <div style={{ background: v.bt?.gradient || C.primary, height: 140, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <div style={{ padding: '14px 16px 16px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                        <div>
                          ...
                          <div style={{ fontSize: 13, color: C.muted }}>{v.bt?.label} · {timeAgo(topItem.at)}</div>
                        </div>
                        {v.hotItems.length > 1 && (
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                        {v.hotItems.map(it => {
                          const iq = QUANTITIES.find(q => q.id === it.quantity);
                          return (
        ...
        {tab === 'venue' && (
          <>
            ...
            <div style={{ background: C.black, padding: '52px 20px 24px' }}>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', fontWeight: 600, marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                {myVenue ? myVenue.icon + ' ' + myVenue.name : 'Setup'}
              </div>
              <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}>
                {myVenue ? 'Dashboard' : 'Register Venue'}
              </h1>
            </div>
              ...
              {(!myVenue || venueScreen === 'register') && (
                <div style={{ background: C.card, borderRadius: 20, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                  <p style={{ fontSize: 14, color: C.muted, margin: '0 0 18px' }}>
                    Register your venue to start announcing fresh batches in real time.
                  </p>
                  <label style={labelStyle}>Type of venue</label>
                  ...
                  <>
                    ...
                    <div style={{ background: C.card, borderRadius: 20, padding: 20, marginBottom: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
                        ...
                        <div style={{ fontSize: 17, fontWeight: 800 }}>Announce a fresh batch</div>
                      </div>
                      ...
                      {justDone
                        ? <div style={{ background: '#E8F5E9', borderRadius: 12, padding: 14, textAlign: 'center', color: '#1A8917', fontWeight: 700, fontSize: 14 }}>✅ Live! Customers can see it now.</div>
                        : <button disabled={saving} onClick={announce} style={btnStyle(C.primary)}>{saving ? 'Publishing…' : '📣 Out of the oven NOW!'}</button>
                      }
                    </div>
                    ...
                    <div style={{ background: C.card, borderRadius: 20, padding: 20, marginBottom: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: myItems.length ? 16 : 0 }}>
                        <div style={{ fontSize: 17, fontWeight: 800 }}>Live right now</div>
                      ...
                      {myItems.length === 0
                        ? <p style={{ fontSize: 14, color: C.muted, margin: '8px 0 0' }}>Nothing announced yet. Hit the button above!</p>
                        : myItems.map((it, idx) => {
                              ...
                              <div key={it.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: idx > 0 ? 12 : 0, marginTop: idx > 0 ? 12 : 0, borderTop: idx > 0 ? `1px solid ${C.border}` : 'none' }}>
                                <div style={{ flex: 1 }}>
                                  ...
                                  <div style={{ fontSize: 12, color: C.muted }}>
                                    <span style={{ color: f.color, fontWeight: 600 }}>{f.label}</span> · {timeAgo(it.at)}
                                  </div>
                    ...
                    <button onClick={() => { setMyVenue(null); localStorage.removeItem('hn_mine'); setVenueScreen('register'); }}
                      style={btnStyle('transparent', true)}>Switch venue</button>
                  </>
                );
              })()}
            </div>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 480, background: C.black, display: 'flex', borderTop: '1px solid rgba(255,255,255,0.08)', zIndex: 100 }}>
        {[
          { id: 'explore',   icon: '🔥', label: 'Discover' },
          { id: 'favorites', icon: '❤️', label: 'Favorites' },
          { id: 'venue',     icon: '🏪', label: 'My Venue'  },
        ].map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            ...
            <span style={{ fontSize: 11, fontWeight: 700, color: tab === t.id ? 'white' : 'rgba(255,255,255,0.35)', fontFamily: 'inherit' }}>
              {t.label}
            </span>
            {tab === t.id && <span style={{ width: 4, height: 4, borderRadius: '50%', background: C.primary, marginTop: -2 }} />}
          </button>
        ))}
      </div>
    </div>
...
function btnStyle(bg, ghost = false) {
  return {
    ...
    background: ghost ? 'transparent' : (bg === 'transparent' ? 'transparent' : `linear-gradient(135deg, #FF5000, #FF8C42)`),
